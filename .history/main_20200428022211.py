...
# print('printing RSS list:')
# for i,j in zip(rssList()[0],rssList()[1]):
#     print(i,j)
##################################
...
def listMatches():
    ...
    while i < len(watchList()):
        if str(watchList()[i]) in str(rssList()[0]):
            res = [x for x in rssList()[0] if re.search(watchList()[i], x)]
            matchList.append(res)
            i = i + 1
    updatedList = []
    j = 0
    while j < len(downloadList()):
        if str(downloadList()[j]) in str(matchList):
            res2 = [x for x in matchList if re.search(downloadList()[j], x)]
            updatedList.remove(res2)
            j = j + 1
listMatches()
##################################