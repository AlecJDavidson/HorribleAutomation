...
### Download List ###
def download_list():
	downloadlist = open(root+'history.txt','r') # Opens downloadlist to read
	downloadlistList = [] # Creates a list file in python to hold entries
	for downloadlistEntry in downloadlist: # Iterates over download list to add new entries
		downloadlistList.append(downloadlistEntry.strip()) # Adds new entries
		downloadlistList = list(dict.fromkeys(watchlistList)) # Converts to dictionary to remove duplicates
    ...
    for link in filter_match()[0]:
        for entry in download_list():
            if entry in link:
                ...
                print('no match')
    i += 1
    return filter_downloads()
print(filter_downloads())
