...
### Update RSS list ###
def rssFeed():
	rssFeed = feedparser.parse(horribleSubsRSS)
	entry = rssFeed.entries[1]
	rssList = [[],[]]
	i = 0
	while i < len(rssFeed.entries):
		rssList[0].append(str('Title :',entry.title))
		rssList[1].append(str('Link :',entry.link))
		print('\n')
		i = i + 1
