...
def rssFeed():
	...
	entriesList = zip(rssEntries[0],rssEntries[1])
	return(entriesList)
...
def findMatch():
	...
	while i < len(watchEntries):
		res = [i for i in rssEntries if watchEntries[i] in i]
		res = list(filter(lambda x: watchEntries in x, test_list)) 
		print(str(watchEntries[i]))
		i += 1
